...
class _MemberTeacherScreenState extends State<MemberTeacherScreen> {
  ...
  Widget build(BuildContext context) {
    final classroom = context.watch<ClassroomViewsModels>();
    ...
    else if (students == null) {  // ✅ First check if students is null
      return Scaffold(
        body: const Center(child: Text("Data is null")),
      );}
    else if ( students.data.isEmpty) {
      return Scaffold(
        body: const Center(child: Text("Data null" )),
      );
    }else if ( classroom.errorMessage != null) {
      return Scaffold(
        body: Center(child: Text("error : ${classroom.errorMessage}...also fetching status : ${students.status}")),
      );
    }else{
      return Container(
        child: Column(
    ...
    //             // Container(child: Text("Theres data ${students.data.first.className}")),
...
class _ClassroomTeacherLayoutState extends State<ClassroomTeacherLayout> {
  ...
  void initState() {
    ...
    Future.microtask(
      () => context.read<ClassroomViewsModels>().getClassDetailById(
  ...
  Widget build(BuildContext context) {
    ...
    if (classroomViewsModels.isLoading) {
      return Scaffold(
        body: Center(
          child: CircularProgressIndicator(
            color: Theme.of(context).primaryColor,
    ...
    if ( classroom == null ||classroom.data.isEmpty) {
      return Scaffold(
        body: const Center(child: Text("You have not created class yet")),
      );
    }
    ...
    return Scaffold(
      body: Column(
        children: [
          ...
          Expanded(
            child: ClipRect(
              // ✅ Add ClipRect to contain the animation
